...
<div class="wrapper row3">
  <div class="col col-lg-10 col-sm-12 col-12 offset-lg-1 working_div">
    <div class="container-fluid home_main center">
      <br><br>
	    <h2 class="text-black" align="center" style="margin-top: 20px; padding:10px 0px 20px 0px;"><b>Show Expenditure</b></h2>
	    <center>
    	 @php
    		 $data=DB::table('expenditure')	   
    			 ->select(DB::raw("CONCAT_WS('-',MONTHNAME(date),YEAR(date)) as monthyear"), DB::raw('sum(rent) as rent_t'), DB::raw('sum(travelling_cost) as travel_t'), DB::raw('sum(employee_salary) as employee_t'), DB::raw('sum(other) as other_t'), DB::raw('sum(food) as food_t'),DB::raw('sum(rent + employee_salary + travelling_cost + other + food) as total_t'))
    			 ->groupby('monthyear')
        ...
        <div class="table-responsive col col-lg-12 col-sm-12 col-12" style="text-align: center; margin-top: 10px;">          
		      <table id="myTable" class="table">
      			...
      			<tbody>
			        ...
			        <tr>
        				<td>{{$cost->monthyear}}</td>
        				<td>{{$cost->rent_t}}/-</td>
        				<td>{{$cost->employee_t}}/-</td>
        				<td>{{$cost->travel_t}}/-</td>
        				<td>{{$cost->food_t}}/-</td>
        				<td>{{$cost->other_t}}/-</td>
        				<td>{{$cost->total_t}}/-</td>
        				<td><a href="{{url('/d_view_expenditure?monthyear='.$cost->monthyear) }}"><i class="fa fa-eye grey-text" style="font-size:16px;margin-right:10px;"></i></a></td>
			        @endforeach
			        </tr>
			      </tbody>
		      </table>
