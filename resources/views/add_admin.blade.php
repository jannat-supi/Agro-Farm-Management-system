  ...
  <div class="col col-lg-10 col-sm-12 col-12 working_div">
    <center>
      ...
      <div class="col col-lg-8 col-sm-12 col-12 offset-lg-2 border shadow" >
        <h2 class="text-black" align="center" style="margin-top: 5%; color: #FF8F2B;"><center><b>Add Admin</b></center></h2>
        <br>
        ...
        <div class="col col-lg-10 col-sm-12 ">
          <form action="{{url('/adding_admin') }}" method="post" >
					    <input type="hidden" name="_token" value="{{csrf_token()}}">
              <div class="form-group row">
                <div class="col-sm-4">
                  <label for="id" style="font-size: 16px;"><b>Select-Admin:</b></label>
                </div>
                <div class="col-sm-8">
                    <select class="form-control" id="user" name="user">
							      @php
			                  $result= DB::table('user')
							        ->where('user_type','Shareholder')
									                  ->get();
			              @endphp
							          <option value="" disabled selected>Select </option>
							          @foreach($result as $user)
								        <option value="{{$user->user_id}}" >{{$user->user_name}}->{{$user->mobile_no}}</option>
					              @endforeach
							      </select>
                </div>
              </div>
              <div class="form-group row">
                <div class="col-sm-12" align="center">
                  <button type="submit" class="btn text-white" name="add" style="background: #FF8F2B;">Add Admin</button>
                </div>
              </div>
              <br>
          </form>
